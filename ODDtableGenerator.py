...
endVal = IntVar()
r10 = Radiobutton(rooty_boi, text="Till 250", variable=endVal, value=250)
r10.grid(row=1, column=6, columnspan=1)

r20 = Radiobutton(rooty_boi, text="Till 350", variable=endVal, value=350)
r20.grid(row=2, column=6, columnspan=1)

r30 = Radiobutton(rooty_boi, text="Till 450", variable=endVal, value=450)
...
def generateMulDinos():
    ...
    for oi in range(endVal.get() + 1):
        tables += str(theTum.get()) + "x" + str(counter) + "=" + str(theTum.get()*counter) + "\n"
        counter += 2
